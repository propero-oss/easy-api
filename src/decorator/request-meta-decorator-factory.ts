...
export type RequestMetaGenerator<T = unknown> = (req: Request, res: Response, next: NextFunction, context: unknown) => T;
...
export function createInjectorMiddleware(
  ...
  return async function (this: unknown, req, res, next) {
    try {
      const parameters = await Promise.all(meta.map((extract) => extract(req, res, next, instance ?? this)));
